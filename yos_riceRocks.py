# http://www.codeskulptor.org/#user27_LaTcFNN2KD_27.py
# Yossi. 12/14/2013.
...
explosion_image2 = simplegui.load_image("http://commondatastorage.googleapis.com/codeskulptor-assets/lathrop/explosion_blue2.png")
...
class Ship:
    ...
    def explode(self):
        ...
        an_explosion = Sprite(self.pos, [0,0], self.angle, 0, explosion_image2, explosion_info, explosion_sound)
...
def draw(canvas):
    ...
    canvas.draw_text("December 2013, Coursera Final Project by YOSSI HAI",[WIDTH / 2 - 190, 20], 17, "Gray")
    ...
    #check if ship collide with rock
    if group_collide(rock_group, my_ship) > 0:
        lives -= 1
        my_ship.explode()
        print "boom! ship collide with rock"
...
def process_sprite_group(group_name, canvas_name):
    for sp in group_name:
        sp_to_remove = set([])
        sp.draw(canvas_name)
        #build set of sprite to remove
        if sp.update() == False:            
            sp_to_remove.add(sp)
        group_name.difference_update(sp_to_remove)


#helper function to hanale ship collision 
...
#helper function to handle missile/rock collision
...
def rock_spawner():
    ...
    if started:
        if len(rock_group)<number_of_rocks:
            ...
            print "adding rock",len(rock_group)
